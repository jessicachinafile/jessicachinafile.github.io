...
# TODO have each entry write into a CSV as it's scraped -- so you can trouble-shoot personpage errror messages.  Right now you can't test because the connection keeps breaking.
...
for province_officials_dict in all_prov_officials_dictlist:
    ...
    for province_official_name, holder_dict in province_officials_dict.items() :
        ...
        ## TODO WHEN THIS IS IMPORTED TO THE FULL SCRAPER, A PersonID COUNTER WILL NEED TO BE ADDED ABOVE AND BELOW, AND THE PersonID WILL NEED TO BE ADDED TO THE IMAGE NAME
        '''
        if len(str(PersonID_counter) = 1 :
            PersonID = '0000000' + PersonID_counter
        elif len(str(PersonID_counter) = 2 :
            PersonID = '000000' + PersonID_counter
        elif len(str(PersonID_counter) = 3 :
            PersonID = '00000' + PersonID_counter
        elif len(str(PersonID_counter) = 4 :
            PersonID = '0000' + PersonID_counter
        elif len(str(PersonID_counter) = 5 :
            PersonID = '000' + PersonID_counter
        elif len(str(PersonID_counter) = 6 :
            PersonID = '00' + PersonID_counter
        elif len(str(PersonID_counter) = 7 :
            PersonID = '0' + PersonID_counter
        else PersonID = PersonID_counter
        '''
        ...
        prov_official_row = pd.DataFrame(prov_official_csv_holder, index=[PersonID], columns=['Name','Title 1', 'Title 2', 'Title 3', 'Title 4', 'Gender', 'Ethnicity', 'Birth Year', 'Birth Month', "Birth Province", "Birth Place", "Education", "Archived", "Scraped", "Image URL", "Image Filename", "Resume"])
...
for citypage_nameonly_cleaned, city_holder_dict in citypage_officials_dict.items() :
    ...
    print("oh my lord why does it take so long to run the city-level officials?")


######### GETTING SCRAPER TO ACCESS COUNTY LEVEL OFFICIAL DICTIONARIES
